...
Mulai dari section ini kita akan menggunakan data `sale_ts`
...
```{r}
#Coba differencing data dan lihat perbedaannya
...
```{r}
#Tuliskan code untuk melakukan forecast data sebanyak n periode menggunakan model_ar & simpan ke dalam variabel fc_ar di bawah sini
```
...
Apakah asumsi residual dari model tersebut terpenuhi? Jika tidak, menurut anda apakah penyebabnya?
...
```{r}
#Tuliskan code anda di bawah sini (tidak perlu menyimpan percobaan anda ke dalam suatu variabel)
```
...
```{r}
#Tuliskan code untuk menghitung forecast n_1 periode menggunakan model_ar_1 (simpan ke dalam variabel fc_ar_1)
...
https://www.kaggle.com/podsyp/time-series-starter-dataset (Monthly Sale.csv) with some adjustment
